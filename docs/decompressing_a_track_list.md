...
context.seek(sample_time, sample_rounding_policy::none, sample_looping_policy::non_looping);
...
You can seek anywhere in a track list but you will need to handle looping manually in your game engine (e.g. by calling `fmod`). When seeking, you must also provide a `sample_rounding_policy` and a `sample_looping_policy` (see also [how to handle looping playback](handling_looping_playback.md)) to dictate how the interpolation is to be performed. See [here](../includes/acl/core/interpolation_utils.h) for details.
