...
TEST_CASE("quat math 32", "[math][quat32]")
{
...

	{
		...
		REQUIRE(vector_all_near_equal(result, vector_set(-1.0f, 0.0f, 0.0f), threshold));
...

	{
		...
		REQUIRE(vector_all_near_equal(axis, vector_set(0.0f, 0.0f, 1.0f), threshold));
		REQUIRE(vector_all_near_equal(quat_get_axis(rotation), vector_set(0.0f, 0.0f, 1.0f), threshold));
