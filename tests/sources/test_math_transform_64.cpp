...
TEST_CASE("transform math 64", "[math][transform64]")
{
...

	{
		...
		REQUIRE(vector_all_near_equal(result, vector_set(1.0, 1.0, 0.0), threshold));
		result = transform_position(transform_a, y_axis);
		REQUIRE(vector_all_near_equal(result, vector_set(0.0, 0.0, 0.0), threshold));
		...
		REQUIRE(vector_all_near_equal(result, vector_set(0.0, 1.0, -1.0), threshold));
		...
		REQUIRE(vector_all_near_equal(result, vector_set(1.0, 1.0, -1.0), threshold));
