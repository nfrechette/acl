...
TEST_CASE("transform math 32", "[math][transform32]")
{
...

	{
		...
		REQUIRE(vector_all_near_equal(result, vector_set(1.0f, 1.0f, 0.0f), threshold));
		result = transform_position(transform_a, y_axis);
		REQUIRE(vector_all_near_equal(result, vector_set(0.0f, 0.0f, 0.0f), threshold));
		...
		REQUIRE(vector_all_near_equal(result, vector_set(0.0f, 1.0f, -1.0f), threshold));
		...
		REQUIRE(vector_all_near_equal(result, vector_set(1.0f, 1.0f, -1.0f), threshold));
		REQUIRE(vector_all_near_equal(result, transform_position(transform_b, transform_position(transform_a, x_axis)), threshold));
		result = transform_position(transform_ab, y_axis);
		REQUIRE(vector_all_near_equal(result, vector_set(0.0f, 1.0f, 0.0f), threshold));
		REQUIRE(vector_all_near_equal(result, transform_position(transform_b, transform_position(transform_a, y_axis)), threshold));
		result = transform_position(transform_ba, x_axis);
		REQUIRE(vector_all_near_equal(result, vector_set(0.0f, 1.0f, 0.0f), threshold));
		REQUIRE(vector_all_near_equal(result, transform_position(transform_a, transform_position(transform_b, x_axis)), threshold));
		result = transform_position(transform_ba, y_axis);
		REQUIRE(vector_all_near_equal(result, vector_set(0.0f, 0.0f, -1.0f), threshold));
		REQUIRE(vector_all_near_equal(result, transform_position(transform_a, transform_position(transform_b, y_axis)), threshold));
	}
}
